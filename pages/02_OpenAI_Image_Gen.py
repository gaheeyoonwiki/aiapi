...
# 이미지 생성을 위한 텍스트와 생성 이미지를 화면에 표시하는 함수    
def display_results():
    # 저장한 세션 상태 불러오기
...
# 버튼 클릭 시 한글 번역, 상세 설명 추가, 이미지를 생성 실행
if clicked:

    if radio_selected_lang == "한국어":
        translated_text = my_image_gen.translate_text_for_image(input_text)
#         translated_text = "A park in a city with a lake with buildings visible"
    elif radio_selected_lang == "영어":
        translated_text = input_text
    
    if detail_description == 'Yes':        
        resp = my_image_gen.generate_text_for_image(translated_text) # OpenAI ChatGPT에 질문하고 응답 받음
        text_for_image = resp
        image_caption ="상세 설명을 추가해 생성한 이미지"
...
#     image_urls = ["https://cdn.pixabay.com/photo/2015/05/04/10/16/vegetables-752153_960_720.jpg",
...
#                  "https://cdn.pixabay.com/photo/2015/05/04/10/16/vegetables-752153_960_720.jpg"]
    ...
    for k, image_url in enumerate(image_urls):
        ...
        # 다운로드 파일 이름 생성
        now_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S") # 이미지 생성 시간
...
# [이미지 생성] 버튼 혹은 [다운로드 버튼] 클릭 시 화면 표시 함수 실행    
