...
# OpenAI Chat API를 이용해 질의에 답변하는 함수
def generate_text_for_image(text):
    ...
    # 대회 메시지 정의
    user_content = f"Describe the following to create an image.\n {text}"
    messages = [ {"role": "user", "content": user_content} ]
...
# OpenAI Image API(Dall-E2)를 이용해 영어 문장으로 이미지를 생성하는 함수
def generate_image_from_text(text_for_image, image_num=1, image_size="512x512"):    
    # API 키 설정
    
    if len(text_for_image) >= 1000: # 텍스트 크기가 1000개보다 크면 1000개로 제한  
        text_for_image = text_for_image[:1000]
    
    # openai.api_key = os.environ["OPENAI_API_KEY"]
    openai.api_key = st.secrets["OPENAI_API_KEY"]

    response = openai.Image.create(prompt=text_for_image, n=image_num, size=image_size)
